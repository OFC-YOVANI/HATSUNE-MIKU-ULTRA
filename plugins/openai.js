...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  try {
    ...
    const config = {method: 'post', url: 'https://api.openai.com/v1/chat/completions', headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + global.openai_key}, data: JSON.stringify({'model': 'gpt-3.5-turbo', 'messages': [{role: 'system', content: 'Actuaras como un Bot de WhatsApp el cual fue creado por Ofc/Yovani, tu seras MIKU-BOT'}, ...chgptdb]})};
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              try {
                conn.sendPresenceUpdate('composing', m.chat);
                const syms1 = `Actuaras como un Bot de WhatsApp el cual fue creado por Ofc/Yovani, tu seras MIKU-BOT`;
...
handler.command = ['openai', 'chatgpt', 'ia', 'robot', 'openai2', 'chatgpt2', 'ia2', 'robot2', 'Miku', 'MikuBot'];
